...
path = "C:\\Users\\sdavilao\\Documents\\Hollow_Data\\hollow_slopes\\HadsallAvg9AxisStats_0614.xlsx"
...
path = "C:\\Users\\sdavilao\\Documents\\Hollow_Data\\hollow_slopes\\\\HadsallAvg9SideslopeStats_0614.xlsx"
...
# To export new dataframe to an excel file
...
#%%
## Plot hollow slope and side slope
## Y is tan of hollow slope, X is tan of side slope
X= (np.tan(np.deg2rad(comp_df.sideslope_mean)))
Y = (np.tan(np.deg2rad(comp_df.hollow_axis_mean)))

plt.figure()
plt.title('Slope Ratio')
plt.scatter((np.tan(np.deg2rad(comp_df.sideslope_mean))), (np.tan(np.deg2rad(comp_df.hollow_axis_mean))), color = 'black', s = 10)
plt.ylabel('tan($θ_H$)')
plt.xlabel('tan($θ_S$)')
