...
from shapely import affinity  # ✅ make sure this import is here too
...
def create_elliptical_buffer(gdf, width, height):
    # Start with unit circle buffer
    circular_buffers = gdf.geometry.buffer(1)

    # Scale the circle into an ellipse: x = width/2, y = height/2
    ellipses = [affinity.scale(geom, xfact=width/2, yfact=height/2, origin='center') for geom in circular_buffers]

    # Replace geometry
    gdf_buffered = gdf.copy()
    gdf_buffered["geometry"] = ellipses
...
def create_buffer_from_points(input_points_path, output_buffer_path, buffer_distance, target_crs=None):
    ...
    # Create elliptical buffer with custom dimensions
    buffered = create_elliptical_buffer(gdf, width=30, height=12)  # or any values you choose
    buffered.set_crs(gdf.crs, inplace=True)  # Preserve CRS for saving
