...
Cleaned and optimized on Apr 3, 2025
...
def apply_buffer_to_soil_depth(grid, shapefile, buffer_distance, dem_path):
    ...
    with rasterio.open(dem_path) as src:
        dem_crs = src.crs
        if gdf.crs and gdf.crs != dem_crs:
    ...
    # Optional: Visualization for debugging (comment out if not needed)
    # plt.imshow(mask, cmap="gray", origin="upper")
    # plt.title("Buffer Mask Visualization")
    # plt.colorbar(label="Buffer Zone")
    # plt.show()
...
# [REMAINDER OF SIMULATION FUNCTIONS GO HERE...]
# Assume the rest of your plotting functions and run_simulation() are unchanged,
# just cleaned up as discussed in your previous message.
